...
do
  ...
  # -p : prompt on stderr
  # -i : use default buffer val
  read -er -i "$PROJECT_NAME" -p "Enter Project Name Static Website .............: " USER_INPUT
...
find -L ./www -type f -name "*.html" ! -path "*/scratch/*" -print0 |
  ...
  do
    if [[ ! -s "$FILE" ]]; then
      ...
      echo "Uploading HTML Templates to Project Bucket ....: $PROJECT_BUCKET${FILE#.}"
    else continue
    fi
  done
#.............................
...
STACK_ID=$(aws cloudformation create-stack --stack-name "$STACK_NAME" --parameters      \
                ParameterKey=ProjectName,ParameterValue="$PROJECT_NAME"                 \
                ParameterKey=BuildStep,ParameterValue="$BUILD_COUNTER"                  \
                ParameterKey=RootDomainName,ParameterValue="$AWS_DOMAIN_NAME"               \
                ParameterKey=DomainHostedZoneId,ParameterValue="$HOSTED_ZONE_ID"        \
